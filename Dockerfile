...
RUN         wget https://luarocks.org/releases/luarocks-3.11.1.tar.gz && \
            ...
            luarocks install luasql-sqlite3 && luarocks install luasql-mysql MYSQL_INCDIR=/usr/include/mysql && luarocks install luasql-postgres PGSQL_INCDIR=/usr/include/postgresql && luarocks install odbc &&\
            cd .. && \
